...
export default function Catalog({ theme, onToggleTheme }: CatalogProps) {
  ...
  useEffect(() => {
    if (!location.includes('#filtros')) return;
    const el = document.getElementById('filtros');
    el?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, [location]);

  useEffect(() => {
    if (cart.count === 0) setCartOpen(false);
  }, [cart.count]);

  const filtered = useMemo(() => {
    return filterProducts(PRODUCTS, brand, category, query);
  ...
  return (
    <main className="min-h-screen bg-background relative">
      ...
      <section className="pt-32 pb-12 bg-[hsl(var(--surface-0))]">
      ...
      <section className="pb-24 bg-[hsl(var(--surface-0))]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div
            id="filtros"
            className="rounded-2xl border border-[hsl(var(--surface-3))] bg-[hsl(var(--surface-1))] p-5 sm:p-6 shadow-sm scroll-mt-28"
          >
            ...
            {hasActiveFilters && (
              <div className="flex flex-wrap items-center gap-2 mt-5 pt-5 border-t border-[hsl(var(--surface-3))]">
                ...
                {category !== ALL && (
                  <FilterChip label={categoryLabel(category)} onRemove={() => setCategory(ALL)} />
                )}
